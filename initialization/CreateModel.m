...
% This code provides the numerical expressions of the state space matrices
% A and B, from their paramatrized expressions (generated by means of the
% code Port2PortCoupledDynamicsPirat). This code allows the user to specify
% its input parameters (mass, geometry, initial state,...).

% This computation has been done once for all and the symbolic expressions
% of A and B are provided in ./initialization/6dof-pirat-model.mat


% OUTPUTS (2 files, stored in the ./initialization subfolder) :
% * linear_model.mat : contains the evaluations of matrices A and B
%                        (numeric values)
% * parameters.mat : contains the numerical values of the parameters that
%                      have been chosen by the user by modifying this file.
%                      By that way, the execution of the GNC loop will be
%                      consistent with respect to these parameters.
...
% Loading Pirat generated matrices (by the function
% Port2PortCoupledDynamicsPirat.m)
load("./initialization/6dof-pirat-model");
