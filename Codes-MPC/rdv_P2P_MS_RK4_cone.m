...
% Cone constraints : (ax + by + cz)/( sqrt((a²+b²+c²)*(x²+y²+z²)) ) -
% cos(phi) < 0  -> ALSO STATE CONSTRAINTS
args.lbg(n_states*(N+1)+1 : n_states*(N+1)+(N+1)) = 0;
args.ubg(n_states*(N+1)+1 : n_states*(N+1)+(N+1)) = phi;
